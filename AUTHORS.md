...
**GitHub Community / Lean community (not a second author list)**
- GitHub Insights → **Community** is the health checklist; it shows `CONTRIBUTING.md`, not co-authors.
- GitHub Insights → **Contributors** follows git author email. That is `frohmanbenjamin@gmail.com` → [@BenFrohman](https://github.com/BenFrohman). No `Co-authored-by` for Grok or xAI.
- Lean Zulip / mathlib `Authors:` / a mathlib4 PR author: Benjamin Stanley Frohman / @BenFrohman. Mathlib wants AI *disclosure* on a PR (`LLM-generated` when substantial), not an AI name on `Authors:`.

**Git identity (do not rewrite history)**
- Commit email: `frohmanbenjamin@gmail.com`
- Historical commit names `Frohmanian` and `BenFrohman` map to **Benjamin Stanley Frohman** in [`.mailmap`](.mailmap). `git shortlog` and `git log --use-mailmap` show the canonical name. Existing SHAs are left unchanged.

See `COPYRIGHT.md` and the per-module five-line headers. For history and priority, see `HISTORY.md`.
